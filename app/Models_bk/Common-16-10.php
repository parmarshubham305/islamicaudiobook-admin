...
class Common extends Model
{
    ...
    {
        $q = Video::where('name', 'like', '%' . $name . '%')->with('user','category','artist')->latest()->get()->toArray();
        return $q;
    }
    ...
    public function check_is_buy($user_id =0)
    {
        $expiry = Transaction::where('status', 1)->get();
        for ($i = 0; $i < count($expiry); $i++) {
            if ($expiry[$i]['expiry_date'] < date('Y-m-d')) {
    ...
    {
        $data = [];
        $total_comment = Comment::where('video_id', $id)->count();
        $total_like = Like::where('video_id', $id)->where('status', 1)->count();
        $total_dislike = Like::where('video_id', $id)->where('status', 2)->count();

        $avg_rating = Comment::where('video_id', $id)->avg('rating');
        ...
        $data['is_favorite'] = "0";
        if ($user_id) {
            $is_favorite = Bookmark::where('video_id', $id)->where('user_id', $user_id)->first();
            if ($is_favorite) {
                $data['is_favorite'] = "1";
            }
        }
        ...
        $data['is_download'] = "0";
        if ($user_id) {
            $is_favorite = Download::where('video_id', $id)->where('user_id', $user_id)->first();
            if ($is_favorite) {
                $data['is_download'] = "1";
            }
        }
