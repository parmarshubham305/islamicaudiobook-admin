                            ...
                            <div class="form-group col-lg-6">
                                <label for="type">{{__('label.video_upload_type')}}</label>
                                    <select class="form-control" name="video_type" id="type">
                                        <option value="server_video" {{ $data->video_type == "server_video" ? 'selected' : ''}}>{{__('label.server_video')}}</option>
                                        <option value="url" {{ $data->video_type == "url" ? 'selected' : ''}}>{{__('label.external_url')}} </option>
                                        <option value="youtube" {{ $data->video_type == "youtube" ? 'selected' : ''}}>{{__('label.youtube_video')}}</option>
                                        <option value="vimeo" {{ $data->video_type == "vimeo" ? 'selected' : ''}}>{{__('label.vimeo_video')}}</option>
                                    </select>
                            </div>
        
                            <div class="youtube vimeo url box form-group col-lg-6 video_url">
                                <label for="type">{{__('label.urls')}}</label>
                                @if($data->video_type =="url" || $data->video_type =="youtube" || $data->video_type =="vimeo")
                                    <input type="text" name="url" id="video_url1" class="form-control" value="{{$data->url}}">
                                @else
                                    ...
                                    <label for="input-1">{{__('label.upload_video')}}</label>
                                        <div id="filelist"></div>
                                            <div id="container" style="position: relative;">
                                                ...
                                                <input type="hidden" name="video_url" id="mp3_file_name" class="form-control">
                                                <div class="form-group">
                                                    @if($data->video_type =="youtube" || $data->video_type =="vimeo")
                                                        <iframe width="200" height="150" src="{{$data->url}}" frameborder="0" allowfullscreen></iframe>
                                                    @else
