            ...
            <div class="card custom-border-card mt-3">
                <div class="card-body">
                    <form enctype="multipart/form-data" id="smart-collection" autocomplete="off">
                        @csrf
                        <input type="hidden" name="id" value="{{ $smartCollection->id ?? '' }}">
                                ...
                                <div class="form-row">
                                    <div class="col-12 mb-3">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="title" class="form-control" placeholder="{{__('label.enter_smart_collection_name')}}" value="{{ $smartCollection->title ?? '' }}">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="col-6 mb-3">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="price" id="priceInput" class="form-control" placeholder="{{__('label.enter_price')}}" value="{{ $smartCollection->price ?? '' }}">
                                        </div>
                                    </div>
                                    <div class="col-6 mb-3">
                                        <div class="form-group">
                                ...
                                <div class="form-row">
                                    <div class="col-12 mb-3">
                                        <div class="form-group">
                                            ...
                                            <textarea name="description" class="form-control" rows="3" placeholder="{{__('label.enter_description')}}">{{ $smartCollection->description ?? '' }}</textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-row">
                                    ...
                                    <div class="col-md-6"> 
                                        <div class="form-group">
                                            <div class="custom-file ml-5"> 
                                                <img  src="{{$smartCollection->image_url ?? asset('assets/imgs/no_img.png')}}" height="120px" width="120px" id="Uploaded-Image">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="form-row">
                                    <div class="col-3 mb-3">
                                        <div class="form-group">
                                            ...
                                            <option value="audio_book" {{ ($smartCollection->type ?? '') == 'audio_book' ? 'selected' : '' }}>{{ __('label.audio_book') }}</option>
                                            <option value="e_book" {{ ($smartCollection->type ?? '') == 'e_book' ? 'selected' : '' }}>{{ __('label.e_book') }}</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-3 mb-3">
...
@section('pagescript')
    ...
    <script>
        $(document).ready(function () {
            ...
            const smartCollectionId = $('#smart-collection input[name="id"]').val();

            let smart_collectio_status = "{{ $smartCollection->status ?? '' }}";
            console.log("smart_collectio_status : ", smart_collectio_status);

            if (smartCollectionId && smart_collectio_status != "") {
                $("#statusSelect").val(smart_collectio_status);
            } else {
                $("#statusSelect").val(1);
            }
            ...
            function initializeEbookTable() {
                if (ebookTable === null) {
                    ...
                    ebookTable.on('draw.dt', function () {
                        console.log('Table fully rendered!');
                        
                        
                        console.log("smartCollectionId : ", smartCollectionId);
                        if (smartCollectionId) {
                            fetchSmartCollectionById(smartCollectionId)
                        }
                    });
                }
            }
            ...
            function initializeAudioBookTable() {
                if (audioBookTable === null) {
                    ...
                    audioBookTable.on('draw.dt', function () {
                        console.log('Table fully rendered!');
            ...
            /**
             * Select specific rows by their IDs in a DataTable.
             *
             * @param {object} tableInstance - The DataTable instance.
             * @param {array} ids - Array of IDs to select.
             * @param {string} checkboxSelector - jQuery selector for checkboxes inside rows.
             * @param {string} dataAttr - Attribute name used for matching (e.g., 'data-id').
             */
            function selectRowsByIds(tableInstance, ids, checkboxSelector = 'input[type="checkbox"]', dataAttr = 'data-id') {
                const allRows = tableInstance.rows({ page: 'all' }).nodes();

                console.log("allRows -- : ", allRows.length);
                $(allRows).each(function () {
                    const rowId = $(this).find(checkboxSelector).attr(dataAttr);
                    if (ids.includes(Number(rowId))) {
                        // Check the checkbox
                        $(this).find(checkboxSelector).prop('checked', true);

                        // Select the row in DataTables (if row selection is enabled)
                        tableInstance.row(this).select();
                    } else {
                        // Uncheck and deselect others
                        $(this).find(checkboxSelector).prop('checked', false);
                        tableInstance.row(this).deselect();
                    }
                });
            }

            function fetchSmartCollectionById(id) {
                $.ajax({
                    url: "{{ route('smart-collection.get', ':id') }}".replace(':id', id),
                    method: 'POST',
                    headers: {
                    ...
                    success: function(response) {
                        if (response.status) {
                            let smart_collection_data = response.data;
                            // console.log("smart_collection_data : ", smart_collection_data);

                            // let titleValue = $('#smart-collection input[name="title"]').val(smart_collection_data.title ?? "");

                            // // Get price value
                            // let price = $('#smart-collection input[name="price"]').val(smart_collection_data.price ?? "");

                            // // Get status selected value
                            // let status = $('#smart-collection select[name="status"]').val(smart_collection_data.status ?? "");

                            // // Get description textarea value
                            // let description = $('#smart-collection textarea[name="description"]').val(smart_collection_data.description ?? "");

                            
                            // console.log("smart_collection_data.items : ", smart_collection_data.items);

                            let item_ids = smart_collection_data.items.map((item) => {
                                return item.item_id;
                            });

                            // console.log("item_ids : ", item_ids);

                            // console.log("smart_collection_data.type : ", smart_collection_data.type);
                            $("#smart-collection-by-select").prop("disabled", true);

                            // setTimeout(() => {
                                if (smart_collection_data.type == "audio_book") {
                                    selectRowsByIds(audioBookTable, item_ids);
                                } else if(smart_collection_data.type == "e_book") {
                                    selectRowsByIds(ebookTable, item_ids);
                                }    
                            // }, 1000);
                            ...
                            // if (smart_collection_data.image_url) {
                            //     $("#Uploaded-Image").attr('src', smart_collection_data.image_url)
                            // }
                        } else {
            ...
            function createSmartCollectionAjax() {
                ...
                console.log("selected_item_ids : ", selected_item_ids);
                // return false;
                ...
                const API_ROUTE = smartCollectionId ? '{{ route('smart-collection.update-data') }}' : '{{ route('smart-collection.create') }}';
