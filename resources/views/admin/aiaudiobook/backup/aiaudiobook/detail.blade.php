...
@section('page_title',  __('Audio Details'))
  ...
  <div class="border-bottom row mb-3">
    <div class="col-sm-10">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('video.index') }}">{{__('label.video')}}</a></li>
  ...
  <div class="card custom-border-card">
        <table class="table table-striped table-hover table-bordered w-75 text-center" style="margin-left:auto; margin-right:auto">
            ...
            <tbody >
                ...
                <tr>
                <tr>
                    <td>{{__('label.video_upload_type')}}</td>
                    <td>
                        @if($detail->video_type)
                            {{$detail->video_type}}
                        @else 
                            -
                        @endif
                ...
                <tr>
                    <td>{{__('label.video_url')}}</td>
                    <td>
                        @if($detail->video_type == "server_video")
                            {{ $detail->url}}

                           
                        @else ($detail->video_type == "url" || $detail->video_type == "youtube" || $detail->video_type == "vimeo")
                            {{$detail->url}}
                        @endif
                    </td>
                </tr>
