@extends('layout.page-app')
    ...
    @section('page_title',  __('Edit Audio Book'))
    ...
    @section('content')
        ...
        <div class="right-content">
            ...
            <div class="body-content">
                <!-- mobile title -->
                <h1 class="page-title-sm">{{__('Edit Audio Book')}}</h1>
                <div class="border-bottom row mb-3">
                    ...
                    <div class="col-sm-2 d-flex align-items-center justify-content-end">
                        <a href="{{ route('aiaudiobook.index') }}" class="btn btn-default mw-120" style="margin-top:-14px">{{__('Audio Book List')}}</a>
                    </div>
                </div>
                        ...
                        <div class="form-row">
                            <div class="col-md-12 mb-3">
                                <div class="form-group">
                                    ...
                                    <a href="{{url('storage/documents')}}/{{$data->upload_file}}" target="_blank">View uploaded file</a>
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="col-md-12 mb-3">
                                <ul class="nav nav-tabs" id="myTab" role="tablist">
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link {{$aiaudioActive}}" id="aiaudio-tab" data-toggle="tab" href="#aiaudio" role="tab" onclick="checkActiveTab(this)" aria-controls="aiaudio"
                                ...
                                <div class="tab-content" id="myTabContent">
                                                ...
                                                @if($data->isAudioTab == 0)
                                                    @if(!empty($audios))
                                                        @foreach($audios as $audio)
                                                            <div class="audio-container" data-id="{{$audio->id}}">
                                                                <input type="text" style="width: 200px; margin-right: 5px; border: 0px solid;" readonly="true" value="{{$audio->audio_name}}">
                                                                <audio style="width: 250px;" controls>
                                                                    <source src="{{url('audio')}}/{{$audio->upload_file}}" type="audio/mpeg">
                                                                </audio>
                                                                <button style="margin-right: 5px;" type="button" value="{{$audio->id}}" class="db-remove-audio">-</button>
                                                            </div>
                                                        @endforeach
                                                    @endif
                                                @endif
                                    ...
                                    <div class="tab-pane fade {{$aiaudioActive}}" id="aiaudio" role="tabpanel" aria-labelledby="aiaudio-tab">
                                        <div class="wrapper" style="margin-top:44px;">
                                            <div class="form-row">
                                                <div class="col-md-12 mb-3">
                                                    <div class="form-group">
                                                        <label>{{__('label.audio_content')}}</label>
                                                        <textarea name="audio_content" id="audio_content" class="form-control" rows="3" placeholder="Enter Content ...">{{$data->audio_content}}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="form-row">
                                                ...
                                                <div class="col-md-2 mb-3">
                                                    <div id="aiaudio-tab-div">
                                                        @if($data->isAudioTab == 1)
                                                            <input type="hidden" name="audio" id="audio_converted_url" value="@if($data){{$data->audio}}@endif">
                                                        @endif
                                                    </div>
    ...
    <script>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(response.status == 200){
                           ...
                           //$("#converted_audio").html('<audio style="width: 223px;" controls><source src="'+assetUrl+'" type="audio/mpeg"></audio>');
                        }else{
        ...
        $(document).ready(function() {
            ...
            $('#confirmDeleteBtn').on('click', function() {
                $.ajax({
                    url: "{{ route('aiaudiobook.deleteaudio', ['id' => ':id']) }}".replace(':id', audioIdToDelete), // Adjust the URL as necessary
                    type: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    success: function(response) {
                        if (response.success) {
                            ...
                            $('#ajax-message').text('Audio file successfully removed.').show().delay(3000).fadeOut();
                        } else {
        ...
        function edit_audio_book(){
            ...
            $.ajax({
                type:'POST',
                url:'{{ route("aiaudiobook.update" ,[$data->id]) }}',
                data:formData,
                cache:false,
                contentType: false,
                processData: false,
